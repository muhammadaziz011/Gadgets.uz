...
const CATALOG_TILE_COLORS = ['#EDE9FE','#FCE7F3','#DCFCE7','#FFEDD5','#E0F2FE','#FEE2E2'];
const CATALOG_TILE_INK = ['#7C3AED','#DB2777','#16A34A','#EA580C','#0284C7','#DC2626'];
function renderCatalogList(){
  document.getElementById('catalogList').innerHTML = CATEGORIES.map((c,i)=>{
    const count = PRODUCTS.filter(p=>p.category===c.id).length;
    return `<button class="catalog-row" onclick="showView('listing',{category:'${c.id}'})">
      <span class="catalog-row-icon" style="background:${CATALOG_TILE_COLORS[i%CATALOG_TILE_COLORS.length]};color:${CATALOG_TILE_INK[i%CATALOG_TILE_INK.length]}"><svg class="icon"><use href="#${c.icon}"/></svg></span>
      <span class="catalog-row-name">${c.name}</span>
      <span class="catalog-row-count">${count} ta</span>
      <svg class="icon catalog-row-chev"><use href="#icon-chevron"/></svg>
    </button>`;
  }).join('') + `
    <button class="catalog-row" onclick="showView('listing',{category:'all', wishlistOnly:true})">
      <span class="catalog-row-icon" style="background:var(--accent-dim);color:var(--accent)"><svg class="icon"><use href="#icon-heart"/></svg></span>
      <span class="catalog-row-name">Sevimlilar</span>
      <svg class="icon catalog-row-chev"><use href="#icon-chevron"/></svg>
    </button>
    <button class="catalog-row" onclick="showView('listing',{category:'all'})">
      <span class="catalog-row-icon" style="background:var(--surface-alt);color:var(--ink)"><svg class="icon"><use href="#icon-compare"/></svg></span>
      <span class="catalog-row-name">Barcha mahsulotlar</span>
      <span class="catalog-row-count">${PRODUCTS.length} ta</span>
      <svg class="icon catalog-row-chev"><use href="#icon-chevron"/></svg>
    </button>`;
}
...
function showView(view, opts={}){
  ['home','listing','catalog','detail'].forEach(v=>document.getElementById(v+'-view').classList.toggle('hidden', v!==view));
  state.view = view;
  if(view==='catalog'){
    renderCatalogList();
  }
...
function syncMobileTabbar(view, opts={}){
  ...
  if(!bar) return;
  let active = 'home';
  if(view==='catalog') active = 'catalog';
...
function quickAdd(id, btnEl){
  ...
  else state.cart.push({productId:id, variant:p.variants[0], colorName:p.colors[0].name, qty:1, sel:true});
...
function addToCart(id){
  ...
  else state.cart.push({productId:id, variant:d.variant, colorName:d.color, qty:d.qty, sel:true});
  persistCart();
  renderCart(); showToast(`${p.name} savatga qo‘shildi`);
}
...
const isSel = c => c.sel !== false;
function toggleCartItemSel(idx){ state.cart[idx].sel = !isSel(state.cart[idx]); persistCart(); renderCart(); }
function toggleCartSelectAll(){
  const allSelected = state.cart.every(isSel);
  state.cart.forEach(c=>{ c.sel = !allSelected; });
  persistCart(); renderCart();
}
function removeSelectedCartItems(){
  const hadSelection = state.cart.some(isSel);
  if(!hadSelection){ showToast('Avval o‘chirish uchun mahsulot tanlang.'); return; }
  state.cart = state.cart.filter(c=>!isSel(c));
  persistCart(); renderCart();
}
function renderCart(){
  ...
  const selectRow = document.getElementById('cartSelectRow');
  ...
  selectRow.innerHTML = `
    <button class="cart-select-link" onclick="removeSelectedCartItems()">Tanlanganlarni o‘chirish</button>
    <label class="cart-select-all"><span>Hammasini tanlang</span><span class="check-box ${allSelected?'checked':''}" onclick="toggleCartSelectAll()"><svg class="icon"><use href="#icon-check"/></svg></span></label>
  `;
  wrap.innerHTML = state.cart.map((c,idx)=>{
    const p = PRODUCTS.find(x=>x.id===c.productId);
    const sel = isSel(c);
    return `<div class="cart-item">
      <span class="check-box cart-item-check ${sel?'checked':''}" onclick="toggleCartItemSel(${idx})"><svg class="icon"><use href="#icon-check"/></svg></span>
  ...
  const selectedLines = state.cart.filter(isSel);
  const selectedCount = selectedLines.reduce((s,c)=>s+c.qty,0);
  const subtotal = selectedLines.reduce((s,c)=>{const p=PRODUCTS.find(x=>x.id===c.productId); return s+p.price*c.qty;},0);
  const fullPriceTotal = selectedLines.reduce((s,c)=>{const p=PRODUCTS.find(x=>x.id===c.productId); return s+(p.oldPrice||p.price)*c.qty;},0);
  const discount = fullPriceTotal - subtotal;
  if(selectedLines.length===0){
    document.getElementById('cartFoot').innerHTML = `<p class="cart-note" style="margin:6px 0 14px;">Buyurtma berish uchun mahsulot(lar)ni tanlang.</p>`;
    return;
  }
  document.getElementById('cartFoot').innerHTML = `
    <div class="cart-summary-lines">
      <div class="cart-summary-line"><span>${selectedCount} ta mahsulot</span><span>${fmt(fullPriceTotal)}</span></div>
      ${discount>0?`<div class="cart-summary-line discount"><span>Chegirmalar</span><span>− ${fmt(discount)}</span></div>`:''}
    </div>
    <div class="cart-subtotal"><span>Jami</span><span class="price">${fmt(subtotal)}</span></div>
    <button class="btn btn-primary btn-block" onclick="startCheckout()">Rasmiylashtirish (${selectedCount})</button>
...
function startCheckout(){
  if(!state.cart.some(isSel)){ showToast('Buyurtma berish uchun mahsulot tanlang.'); return; }
  renderCheckoutModal();
...
function renderCheckoutModal(){
  ...
  const subtotal = state.cart.filter(isSel).reduce((s,c)=>{const p=PRODUCTS.find(x=>x.id===c.productId); return s+p.price*c.qty;},0);
...
function confirmOrder(){
  ...
  const selectedLines = state.cart.filter(isSel);
  const total = selectedLines.reduce((s,c)=>{const p=PRODUCTS.find(x=>x.id===c.productId); return s+p.price*c.qty;},0);
  const itemCount = selectedLines.reduce((s,c)=>s+c.qty,0);
  const orders = getOrders();
  const order = { id: String(Date.now()).slice(-6), date: new Date().toLocaleDateString('uz-UZ'), items: selectedLines.slice(), itemCount, total, name, phone, address };
  orders.push(order);
...
let profileModalStep = 'teaser';
function openProfileModal(){
  profileModalStep = 'teaser';
  renderProfileModal();
...
function showProfileLoginForm(){ profileModalStep = 'form'; renderProfileModal(); }
function profileComingSoon(){ showToast('Bu bo‘lim tez orada qo‘shiladi.'); }
function renderProfileModal(){
  const profile = getProfile();
  const body = document.getElementById('profileModalBody');
  if(!profile && profileModalStep==='teaser'){
    body.innerHTML = `
      <div style="text-align:center;padding:6px 0 22px;">
        <div style="width:74px;height:74px;border-radius:50%;background:var(--accent);color:#fff;display:flex;align-items:center;justify-content:center;margin:0 auto 10px;">
          <svg class="icon" style="width:34px;height:34px;color:#fff;"><use href="#icon-user"/></svg>
        </div>
        <span style="display:inline-block;background:var(--accent);color:#fff;font-size:11px;font-weight:700;padding:2px 10px;border-radius:10px;margin-bottom:14px;">Kirish</span>
        <p style="font-size:14px;color:var(--ink-soft);max-width:280px;margin:0 auto;">Profilga kiring va GadgetZone’da xaridlaringizni kuzatib boring!</p>
      </div>
      <div class="profile-list">
        <button class="profile-list-row" onclick="profileComingSoon()"><svg class="icon"><use href="#icon-truck"/></svg><span>Yetkazib berish manzillari</span><svg class="icon chev"><use href="#icon-chevron"/></svg></button>
        <button class="profile-list-row" onclick="profileComingSoon()"><svg class="icon"><use href="#icon-shield"/></svg><span>Bildirishnomalar</span><svg class="icon chev"><use href="#icon-chevron"/></svg></button>
        <button class="profile-list-row" onclick="profileComingSoon()"><svg class="icon"><use href="#icon-compare"/></svg><span>Ma’lumot</span><svg class="icon chev"><use href="#icon-chevron"/></svg></button>
        <button class="profile-list-row" onclick="profileComingSoon()"><svg class="icon"><use href="#icon-heart"/></svg><span>Biz bilan bog‘lanish</span><svg class="icon chev"><use href="#icon-chevron"/></svg></button>
      </div>
      <p style="text-align:center;font-size:11.5px;color:var(--ink-faint);margin:16px 0 14px;">GadgetZone versiyasi: 1.0.0</p>
      <button class="btn btn-primary btn-block" onclick="showProfileLoginForm()">Kirish</button>
    `;
    return;
  }
  if(!profile && profileModalStep==='form'){
    body.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <button class="btn btn-ghost btn-block" onclick="profileModalStep='teaser';renderProfileModal();">Orqaga</button>
      </div>`;
    return;
  }
  {
...
/* ================= HEADER SCROLL DEPTH (desktop premium) ================= */
const siteHeaderEl = document.querySelector('.site-header');
function syncHeaderScrollDepth(){
  if(!siteHeaderEl) return;
  if(window.scrollY > 8) siteHeaderEl.classList.add('scrolled');
  else siteHeaderEl.classList.remove('scrolled');
}
window.addEventListener('scroll', syncHeaderScrollDepth, {passive:true});

/* ================= INIT ================= */
syncHeaderScrollDepth();
renderNav();
renderCatRow();
renderHome();
renderFilterPanel();
